...
class Choice_Block_mb(nn.Module):
    def __init__(self, in_channels, out_channels, kernel, stride, supernet=True):
        super(Choice_Block_mb, self).__init__()
        padding = kernel // 2
        if supernet: self.affine = False
        else: self.affine = True
        self.stride = stride
        self.in_channels = in_channels
        self.mid_channels = in_channels * 6
        ...
        self.cb_main = nn.Sequential(
            ...
            # 3x3 depth wise conv
            nn.Conv2d(self.mid_channels, self.mid_channels, kernel_size=kernel, stride=stride, padding=padding, groups=self.mid_channels, bias=False),
            nn.BatchNorm2d(self.mid_channels, affine=self.affine),
            nn.ReLU6(inplace=True),
            # 1x1 point wise conv
