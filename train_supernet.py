...
# Supernet Settings
parser.add_argument('--layers', type=int, default=16, help='batch size')    #### 20 -> 16
parser.add_argument('--num_choices', type=int, default=3, help='number choices per layer')    #### 4 -> 3
# Training Settings
...
# logging.basicConfig(stream=sys.stdout, level=logging.INFO, format=log_format, datefmt='%m/%d %I:%M:%S %p')    # modified on 01/18
logging.basicConfig(level=logging.INFO, filename='./logdir/mb_train_super.log', filemode='w', format=log_format, datefmt='%m/%d %I:%M:%S %p')
logging.info(args)
